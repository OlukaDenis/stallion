...
// adding a editview button on the derivery page to display uploaded files
  ...
  const fetchData = async () => {
    const query =
      isAdmin || isManager
        ? firebase.firestore().collection('/orders').where('delivered', '==', true)
        : isShippingAgent || isDriver
        ? firebase.firestore().collection('/orders').where('driver_uid', '==', userUID).where('delivered', '==', true)
        : null;
...
DeliveredJobs.getInitialProps = async () => ({
  namespacesRequired: ['common', 'jobs_delivered'],
});

DeliveredJobs.propTypes = {
...
export default connect(mapStateToProps, null)(withTranslation('jobs_delivered')(DeliveredJobs));
