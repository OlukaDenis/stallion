...
describe('quote reducer', () => {
  ...
  it('should handle ' + actions.quoteActionTypes.SET_CARS, () => {
    const action = getActionObjectFromFunction(
      actions.setCars({ 0: { year: '2013', make: 'Toyota', model: 'Mark X' } })
    );
    expect(quoteReducer(quoteInitialState, action)).toEqual({
      ...quoteInitialState,
  ...
  it('should handle ' + actions.quoteActionTypes.SET_IS_PAID, () => {
    const action = getActionObjectFromFunction(actions.setIsPaid(true));
    expect(quoteReducer(quoteInitialState, action)).toEqual({
      ...quoteInitialState,
      isPaid: true,
    });
